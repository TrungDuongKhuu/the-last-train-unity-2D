...
namespace HeneGames.DialogueSystem
{
    public class DialogueManager : MonoBehaviour
    {
        ...
        private void PlaySound(AudioClip _audioClip)
        {
            ...
            // Gán clip mới cho AudioSource và phát nó
        ...
        // Thêm hàm này vào cuối class DialogueManager
        public void StartCustomDialogue(List<NPC_Sentence> customSentences)
        {
            // Nếu đang có hội thoại thì không bắt đầu hội thoại mới
            if (dialogueIsOn)
            {
                Debug.LogWarning("Dialogue is already active. Cannot start a new custom dialogue.");
                return;
            }

            // Gán danh sách câu thoại tùy chỉnh cho trình quản lý
            this.sentences = customSentences;

            // --- PHẦN SỬA LỖI ---
            // Reset lại chỉ số câu và bắt đầu hiển thị câu đầu tiên
            currentSentence = 0;

            // Kích hoạt sự kiện bắt đầu hội thoại chung
            startDialogueEvent.Invoke();

            // Bắt đầu giao diện UI
            DialogueUI.instance.StartDialogue(this);

            // Hiển thị câu đầu tiên (quan trọng: hàm này sẽ trigger sentenceEvent)
            ShowCurrentSentence();

            // Thiết lập thời gian chờ cho câu đầu tiên
            coolDownTimer = sentences[currentSentence].skipDelayTime;

            // Đánh dấu là đang có hội thoại
            dialogueIsOn = true;
            // --- KẾT THÚC SỬA LỖI ---
        }
