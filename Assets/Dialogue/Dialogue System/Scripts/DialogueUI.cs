...
namespace HeneGames.DialogueSystem
{
    public class DialogueUI : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                Destroy(gameObject);
                return; // Dừng ngay để tránh truy cập UI components
            }
        }
        ...
        public void NextSentenceSoft()
        {
             if (startDialogueDelayTimer <= 0f)
            {
                ...
                {
                    // Dừng coroutine chạy chữ
                    StopAllCoroutines();
                    typing = false;
                    messageText.text = currentMessage;

                    // --- THÊM DÒNG NÀY ---
                    // Yêu cầu DialogueManager dừng âm thanh
                    if (currentDialogueManager != null)
                    {
                        currentDialogueManager.StopVoice();
                    }
                    // --------------------
                }
            }
        ...
        IEnumerator WriteTextToTextmesh(string _text, TextMeshProUGUI _textMeshObject)
        {
            ...
            // Khi chữ đã chạy xong, yêu cầu DialogueManager dừng âm thanh
            if (currentDialogueManager != null)
            {
                currentDialogueManager.StopVoice();
            }
