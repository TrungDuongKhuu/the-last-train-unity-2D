...
public class MenuPuzzle2 : MonoBehaviour
{
    ...
    public TextMeshProUGUI textPuzzleProgress; // hoặc TextMeshProUGUI nếu bạn dùng TMP
    ...
    {
        GameManager.Instance.LoadSceneWithFade("Office Room", "Enter_FromPuzzle2");
    }

    public void OnClickPlayAgain()
    {
        if (GameApp.Instance.puzzle2CompletedCount >= GameApp.Instance.puzzle2TargetCount)
        {
            // Đã đủ 5 -> về Office
            GameManager.Instance.LoadSceneWithFade("Office Room", "Enter_FromPuzzle2");
        }
        ...
        {
            // Chưa đủ -> chơi tiếp
            GameManager.Instance.LoadSceneWithFade("Chapter 1 Puzzle 2");
        }
    }


    public void UpdatePuzzleProgressUI()
    {
        if (GameApp.Instance == null || textPuzzleProgress == null) return;

        int current = GameApp.Instance.puzzle2CompletedCount;
        int total = GameApp.Instance.puzzle2TargetCount;

        textPuzzleProgress.text = $"{current}/{total}";
    }
