...
namespace Level
{
    public class LevelLoader : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // 🔥 FIX: Load level từ progress
            LoadSavedOrFirstLevel();
        }

        /// <summary>
        /// Load level theo progress từ PlayerPrefs,
        /// hoặc level 0 nếu progress vượt giới hạn.
        /// </summary>
        private void LoadSavedOrFirstLevel()
        {
            int savedIndex = PlayerPrefs.GetInt(PREF_CURRENT_LEVEL, 0);

            if (savedIndex < 0 || savedIndex >= levels.Length)
                savedIndex = 0;

            _currentLevelIndex = savedIndex;
        ...
        public void LoadNextLevel()
        {
            ...
            if (_currentLevelIndex + 1 >= levels.Length ||
                levels[_currentLevelIndex + 1] == null)
            {
#if UNITY_EDITOR
            ...
            // 🔥 Cập nhật tiến độ
            PlayerPrefs.SetInt(PREF_CURRENT_LEVEL, _currentLevelIndex);
            PlayerPrefs.Save();
        ...
        private void SetCamera(Bounds bounds)
        {
            ...
            float horizontalSize = bounds.size.x * _camera.pixelHeight /
                                   _camera.pixelWidth / 2f;
