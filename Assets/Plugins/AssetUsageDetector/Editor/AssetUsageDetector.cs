            ...
            if (!(obj is ValueType) && (result != null || unityObject != null || currentDepth == 0))
            {
                if (!searchingSourceAsset)
                {
                    if (obj is Object)
                    {
                        ...
                        // 🩵 FIX DUPLICATE KEY: chỉ thêm nếu chưa có
                        if (!searchedUnityObjects.ContainsKey(id))
                            searchedUnityObjects.Add(id, result);
                        else
                            searchedUnityObjects[id] = result; // cập nhật thay vì gây lỗi
                    }
                    else
                    {
                        string key = GetNodeObjectHash(obj);
                        if (!searchedObjects.ContainsKey(key))
                            searchedObjects.Add(key, result);
                        else
                            searchedObjects[key] = result;
                    }
                }
                else if (result != null)
                {
                    int id = unityObject.GetInstanceID();
                    if (searchedUnityObjects.ContainsKey(id) && searchedUnityObjects[id] != null)
                        result.CopyReferencesTo(searchedUnityObjects[id]);
                    PoolReferenceNode(result);
                }
            }
